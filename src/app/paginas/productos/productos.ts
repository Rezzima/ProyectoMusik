...
export class Productos {
  ...
  toggleFavorito(id: number) {
    ...
    // Busca dentro del array el producto cuyo id coincida
    const producto = this.Productos.find(p => p.id === id);

    // Si lo encontró (no es undefined), invierte su estado favorito
    // false -> true (agregar a favoritos)
    // true -> false (quitar de favoritos)
    if (producto) {
